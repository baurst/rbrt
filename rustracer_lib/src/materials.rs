...
impl Lambertian{
    pub fn scatter(&self, incoming_ray: &Ray, hit_info: &HitInformation, attentuation: &mut Vec3,  scattered_ray: &mut Ray)->bool {
        let scattered_ray_target = hit_info.hit_point + hit_info.hit_normal + random_point_in_unit_sphere();
...
pub fn random_point_in_unit_sphere() -> Vec3 {
    let mut point = Vec3::new(rand::random::<f64>(),rand::random::<f64>(),rand::random::<f64>());
    while point.length() > 1.0 {
        point = Vec3::new(rand::random::<f64>(),rand::random::<f64>(),rand::random::<f64>());
    }
    return point;
}
...
pub fn refract(incoming_ray_dir: &Vec3, normal: &Vec3, ni_over_nt: f64, refracted_ray_dir: &mut Vec3) -> bool {
